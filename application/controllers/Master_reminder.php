	...
	class Master_reminder extends CI_Controller
	{
		...
		function notify()
		{
			...
			foreach ($data['fetch_notify_ssl_reminder_db'] as $row) {
				...
				$reminder_gst_amt[] = $row['gst_amt'];
				$reminder_net_amt[] = $row['net_amt'];
			...
			for ($i = 0; $i < count($reminder_name); $i++) { 
				for ($j = 0; $j < count($reminder_company_names); $j++) { 
					...
					$data = array('reminder_name'=>$reminder_name[$i],'reminder_company_names'=>$reminder_company_names[$j],'reminder_websites'=>$reminder_websites[$j],'reminder_amount'=>$reminder_amount[$j], 'reminder_gst_amt'=>$reminder_gst_amt[$j],'reminder_net_amt'=>$reminder_net_amt[$j]);
					// $this->arr_op($data);
					$message = $this->load->view('master_reminder/mail_format', $data, false);
